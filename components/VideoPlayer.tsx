...
export const VideoPlayer: React.FC<CustomVideoPlayerProps> = ({ videoSrc }) => {
  ...
  const handleProgressChange = useCallback((e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Ensure the duration is valid and finite
    if (isNaN(videoDuration) || videoDuration <= 0) {
      console.warn("Invalid video duration:", videoDuration);
      return; // Exit if the duration is not valid
    }

    // Calculate the new current time based on the input value
    const newTime = (Number(e.target.value) / 100) * videoDuration;

    // Set the current time only if the value is finite
    if (isFinite(newTime)) {
      videoRef.current.currentTime = newTime;
      setProgress(Number(e.target.value));
    } else {
      console.warn("Calculated newTime is not finite:", newTime);
    }
  }, []);
  ...
  return (
    <div ref={playerRef} className="flex flex-col justify-center items-center max-w-full -mt-10 relative mb-16">
